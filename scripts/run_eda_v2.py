...
9. **Holidays**: {stats.get('unmapped_gaps', 0)} non-Sunday gaps not in `ALL_HOLIDAYS` — review list below.
...
| Holiday gaps | `is_holiday`, `days_to_next_holiday`, add `is_pre_holiday` |
| Private = October | `days_to_october`, `lag_364`, direct horizon h>28 |
| Recursive drift | Direct multi-horizon + blend α_h |

## Holiday gap audit

- Missing days: **{stats.get('missing_days_total', 0)}** (Sun: {stats.get('missing_sunday', 0)}, non-Sun: {stats.get('missing_non_sunday', 0)})
- Non-Sunday gaps **not** in ALL_HOLIDAYS: **{stats.get('unmapped_gaps', 0)}**
- Sample unmapped dates: {', '.join(unmapped_dates) if unmapped_dates else 'none'}
...
1. Add unmapped gap dates to `ALL_HOLIDAYS` only if recurring (avoid overfitting one-off closures).
2. Pre-holiday dip in chart 11 — quantify before adding `is_pre_holiday` globally.
3. Whale inter-arrival (chart 16) — consider `days_since_last_sale` cap for top tier only.
"""

(EDA_DIR / "EDA_STATS.md").write_text(report, encoding="utf-8")
# Full narrative report maintained in EDA_REPORT.md (see plan checklist)
print(f"Saved stats → {EDA_DIR / 'EDA_STATS.md'}")
print(f"Charts → {EDA_DIR}/ (18 PNG)")
print("Done.")
